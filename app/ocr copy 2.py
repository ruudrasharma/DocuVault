...
def extract_fields(file_path):
    ...
    for i, line in enumerate(text_lines):
        if re.search(r'MARKS OBTAINED|SUBJECT|SUB\. CODE|CODE', line, re.IGNORECASE):
            marks_start = i
    ...
    if marks_start is not None and marks_end is not None:
        ...
        for line in marks_section:
            match = re.match(r'(\d{3})\s+(.+?)\s*[\—:\|]*\s*(\d{3})\s*[\—:\|]*\s*(\d{3})\s*[\—:\|]*\s*(\d{3})\s*[\—:\|]*\s*([A-Z\s]+)\s*[\—:\|]*\s*([A-D][1-9tIlL])', line, re.IGNORECASE)
            if match:
    ...
    # Fallback if not all marks extracted
    if len(result["marks"]) < 6:
        subject_patterns = [
